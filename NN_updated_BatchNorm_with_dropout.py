...
def linear_activation_forward(A_prev, W, B, activation, mode):
    """
    Description:
    ...
    """
    Z, linear_cache = linear_forward(A_prev, W, B, mode)
    A = 0
    ...
    for layer_index, hidden_layer in parameters.items():
        if not isinstance(layer_index, str):
            ...
            if layer_index == len(parameters)-4:
                ...
                activation = 'softmax'
            mode = 'dropout'
            if layer_index == 1 or parameters['mode'] != 'train':
                mode = 'no_dropout'
            A, linear_cache = linear_activation_forward(A_prev, hidden_layer_weights, hidden_layer_bias, activation, mode)
            if use_batchnorm and layer_index != len(parameters)-4:
                ...
                if parameters['mode'] == 'train':
                    ...
                    mean = np.mean(parameters['batchNorm_mean'][layer_index][
                                   max(0, len(parameters['batchNorm_mean'][layer_index]) - 100):], axis=0)
                    std = np.mean(parameters['batchNorm_std'][layer_index][
                                  max(0, len(parameters['batchNorm_std'][layer_index]) - 100):], axis=0)
                    A = A - mean
...
def load_mnist():
    ...
    2. split the train dataset to train and validation datasets (80/20) using sklearn.model_selection.train_test_split
    3. flatten the input data to a matrix of [m,784]
Ass_1.py
